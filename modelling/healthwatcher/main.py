...
v, _, meta = torchvision.io.read_video(
    "../data/S98T89.mp4", pts_unit="sec"
)  ## assumes it's being run from `healthwatcher` directory
print(meta)
fps = meta["video_fps"]
...
spo2_smooth = [
    spo2[i : i + frames_to_smooth].mean() for i in range(len(spo2) - frames_to_smooth)
]
