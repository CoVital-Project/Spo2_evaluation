...
def ppg(frames, timestamps):
    ...
    vps_red = list()
    for frame in frames:
        green_channel_mean, red_channel_mean, = getColorComponentsMean(
            frame, blue=0, green=1, red=2
        )
        vps_green.append(green_channel_mean)
        vps_red.append(red_channel_mean)

    # Spline interpolation
    green_channel_mean_interpolated = scipy.interpolate.CubicSpline(
        timestamps, green_channel_mean
    )
    red_channel_mean_interpolated = scipy.interpolate.CubicSpline(
        timestamps, red_channel_mean
    )
